...
- `internal/client/` — hand-written Go client for vodopad, all `/v3`: `client.go` (resources and catalogs), `vm_draft.go` (VM draft and lifecycle), `vm_interfaces.go`.
...
- **Gotchas**:
  ...
  - **The provider speaks `/v3` and needs the API at 0.14.0 or later.** 0.14 gave every resource a `/v3` twin — kebab-case paths, `Public*` payloads, `DELETE /{id}` in place of the `/v1` bulk-delete POST, paginated lists, its own error body — and the VM surface changed shape outright: one `POST /v3/vms` carries the whole draft and answers with a receipt. `/v1` and `/v2` are no longer used anywhere, and the mock does not serve them.
  - **`Idempotency-Key` is a required header on `POST /v3/vms` and `POST /v3/vms/{id}/terminate`.** Without it the live API answers `400 invalid request` even for an empty body. The contract middleware validates bodies, not headers, so it cannot catch this — only an acceptance run can.
  - A v3 `name` is at most 25 characters and must match `^[a-z0-9]([a-z0-9-]*[a-z0-9])?$`; a rename in a test has to fit too.
  - Live API quirks the public docs still miss: security groups are created by `vpcId` (cluster derived), subnet create needs at least one CIDR, and an IPv6-only subnet must set `egress = false`. Assume every API bump can break a request body; the gate catches it only after `make openapi-refresh`.
  - **The pin follows stage while the move lands.** 0.14 is on stage and mainnet is behind, so `OPENAPI_URL` points at stage and the vendored snapshot is a stage one — against the usual rule, deliberately (graph node #2095). Put both back to mainnet once 0.14 ships there; until then the provider cannot be released.
  - Catalog lists (`/v3/clusters`, `/v3/datacenters`, `/v3/configurations`, `/v3/images`) come wrapped as `{"items": […]}`; the mock contract test GETs them so an envelope change fails offline, not on stage.
