...
- **Gotchas**:
  ...
  - The live API (0.12.0, mainnet and stage alike) is ahead of the public docs: security groups are created by `vpcId` (cluster derived), subnet create rejects `clusterId` (`additionalProperties: false`) and needs at least one CIDR, moving a draft VM needs `expectedUpdatedAt` echoed verbatim. Assume every API bump can break a request body; the gate catches it only after `make openapi-refresh` — 0.12.0 did exactly that to the draft boot disk (`imageId` became a tagged `source`).
  - **Stage runs ahead of mainnet, and the pin is mainnet.** Stage 0.13.0 already expresses the subnet CIDR rules in the schema (`anyOf`, plus `egress:false` for IPv6-only) and answers `422 subnet_cidr_required`; mainnet is 0.12.0 and still answers `400 bad_request`. Diff the two before assuming a rule is only in prose: `curl -fsSL https://api.stage.fluence.dev/docs/fluence-public.yaml`.
