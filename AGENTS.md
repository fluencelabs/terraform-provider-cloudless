...
- **Gotchas**:
  ...
  - The live API (mainnet 0.13.1) is ahead of the public docs: security groups are created by `vpcId` (cluster derived), subnet create rejects `clusterId` (`additionalProperties: false`) and needs at least one CIDR, moving a draft VM needs `expectedUpdatedAt` echoed verbatim. Assume every API bump can break a request body; the gate catches it only after `make openapi-refresh` — 0.12.0 did exactly that to the draft boot disk (`imageId` became a tagged `source`).
  - **Stage runs ahead of mainnet, and the pin is mainnet.** Diff the two before assuming a rule is only in prose: `curl -fsSL https://api.stage.fluence.dev/docs/fluence-public.yaml`. Stage 0.14.0 carries a **second, complete `/v3` surface** for every resource the provider uses (`/v3/vpcs`, `/v3/subnets`, `/v3/security-groups`, `/v3/ssh-keys`, `/v3/storages`, `/v3/public-ips`, and the catalogs), with kebab-case paths, `Public*` DTOs, real `DELETE /{id}`, paginated lists and its own error body. Nothing there is deprecated and the client still speaks `/v1`; whether to follow is the owner's call (graph node #2095) — do not start the move on your own.
  - `/v3` carries the whole VM lifecycle since 0.12.0 (restart, softreboot, terminate, list and get), but three verbs there are **draft-only**: `PATCH /v3/vms/{id}` (name and SKU), `POST /v3/vms/{id}/storages` and `DELETE /v3/vms/{id}/storages/{id}`. Renaming a live VM and adding or removing its data disks stay on `/v2`, as do `GET /v2/vms/{id}/interfaces` and the interface `PATCH` (security group, static IPs) — `/v3` has no equivalent.
